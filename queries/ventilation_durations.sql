...
drop materialized view if exists ventilation_durations cascade;
create materialized view ventilation_durations as
...
select icustay_id
  ...
  , DATETIME_DIFF_MINUTE(max(charttime), min(charttime))/60 AS duration_hours
from vd2
