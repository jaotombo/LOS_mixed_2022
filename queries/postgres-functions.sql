...
CREATE OR REPLACE FUNCTION DATETIME_DIFF_YEAR(endtime TIMESTAMP(3), starttime TIMESTAMP(3)) RETURNS NUMERIC AS $$
BEGIN
RETURN 
    EXTRACT(EPOCH FROM endtime - starttime) /365.242*24*3600.0
    END;
END; $$
LANGUAGE PLPGSQL;


CREATE OR REPLACE FUNCTION DATETIME_DIFF_HOUR(endtime TIMESTAMP(3), starttime TIMESTAMP(3)) RETURNS NUMERIC AS $$
BEGIN
RETURN 
...
CREATE OR REPLACE FUNCTION DATETIME_DIFF_SECOND(endtime TIMESTAMP(3), starttime TIMESTAMP(3)) RETURNS NUMERIC AS $$
BEGIN
RETURN 
    EXTRACT(EPOCH FROM endtime - starttime) /1.0
  	END;
END; $$
LANGUAGE PLPGSQL;

CREATE OR REPLACE FUNCTION DATETIME_DIFF_MINUTE(endtime TIMESTAMP(3), starttime TIMESTAMP(3)) RETURNS NUMERIC AS $$
BEGIN
RETURN 
